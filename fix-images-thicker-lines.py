#!/usr/bin/env python3
"""
Fix images: Pure black and white with thicker, solid lines
"""
...
def thicken_lines(url_key):
    """Convert image to pure black and white with thicker lines"""
    ...
    try:
        ...
        # Increase contrast first to make lines more visible
        enhancer = ImageEnhance.Contrast(img)
        img = enhancer.enhance(2.0)  # Increase contrast significantly
        
        # Apply binary threshold
        # Lower threshold = more black (thicker lines)
        threshold = 140  # Adjusted to capture more detail
        img_array = np.array(img)
        
        # Create binary image (0 = black/line, 255 = white/background)
        binary = np.where(img_array < threshold, 0, 255).astype(np.uint8)
        
        # Dilate (thicken) the black lines
        # Create a dilation structure (cross shape for natural line thickening)
        structure = ndimage.generate_binary_structure(2, 1)
        
        # Invert for dilation (need white lines on black background)
        inverted = 255 - binary
        
        # Dilate to thicken lines (iterations = thickness)
        dilated = ndimage.binary_dilation(inverted > 0, structure=structure, iterations=2)
...
def main():
    print("🎨 Fixing Images: Thicker Lines + Pure Black & White")
    print("="*80)
    ...
    if success == total:
        print("\n✨ All images now have thicker lines and are pure black & white!")
        print("\n📋 Next steps:")
